...
function migrate() {
  try {
    ...
    if (!colsP.some((r) => r[1] === 'original_price')) db.run("ALTER TABLE products ADD COLUMN original_price REAL");
    if (!colsP.some((r) => r[1] === 'sold_count')) db.run("ALTER TABLE products ADD COLUMN sold_count INTEGER NOT NULL DEFAULT 0");
    if (!colsP.some((r) => r[1] === 'purchase_limit')) db.run("ALTER TABLE products ADD COLUMN purchase_limit INTEGER NOT NULL DEFAULT 0");
    if (!colsP.some((r) => r[1] === 'is_hot')) db.run("ALTER TABLE products ADD COLUMN is_hot INTEGER NOT NULL DEFAULT 0");
    ...
    if (!colsU.some((r) => r[1] === 'points')) db.run("ALTER TABLE users ADD COLUMN points INTEGER NOT NULL DEFAULT 0");
    if (!colsU.some((r) => r[1] === 'is_banned')) db.run("ALTER TABLE users ADD COLUMN is_banned INTEGER NOT NULL DEFAULT 0");
    if (!colsU.some((r) => r[1] === 'last_checkin_at')) db.run("ALTER TABLE users ADD COLUMN last_checkin_at TEXT");
    ...
    db.run(`CREATE TABLE IF NOT EXISTS inventory_locks (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      order_id INTEGER NOT NULL UNIQUE,
      product_id INTEGER NOT NULL,
      quantity INTEGER NOT NULL,
    ...
    db.run("CREATE TABLE IF NOT EXISTS payment_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, order_id INTEGER, order_no TEXT, event_type TEXT NOT NULL, payload TEXT, result TEXT, message TEXT, created_at TEXT DEFAULT (datetime('now')), FOREIGN KEY (order_id) REFERENCES orders(id))");
